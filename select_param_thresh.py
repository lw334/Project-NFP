...
def preprocessing(cv_train, cv_test):
	#fill_nans
	for col_name in NANCOLS_CAT_BINARY:
		cv_train = fill_mode(cv_train,col_name)
		cv_test= fill_mode(cv_test,col_name)

	for col_name in NUMERICAL:
		if col_name != 'edd_enrollment_interval_weeks':
			cv_train = fill_median(cv_train,col_name)
			cv_test = fill_median(cv_test,col_name)

	cv_train = fill_median(cv_train,"MotherNurseAgeDiff")
	cv_test = fill_median(cv_test,"MotherNurseAgeDiff")

	# Transforming features 
	cv_train = cat_var_to_binary(cv_train,CATEGORICAL_2)
	cv_test = cat_to_bi_test(cv_test,CATEGORICAL_2,cv_train)

	cv_train = binary_transform(cv_train, BOOLEAN)
	cv_test = binary_transform(cv_test, BOOLEAN)

	create_match_feature("MomsRE_WhiteNH", "nurserace_white", "whiteMatch", cv_train)
	create_match_feature("MomsRE_Hispanic or Latina", "nurse_hispanic", "hispanicMatch", cv_train)
	create_match_feature("MomsRE_BlackNH", "nurserace_black", "blackMatch", cv_train)
	create_match_feature("MomsRE_WhiteNH", "nurserace_white", "whiteMatch", cv_test)
	create_match_feature("MomsRE_Hispanic or Latina", "nurse_hispanic", "hispanicMatch", cv_test)
	create_match_feature("MomsRE_BlackNH", "nurserace_black", "blackMatch", cv_test)
...
def select_parameter_bstp(df, splits, classifier, y_col, dic_param_vals, list_threshold, criterion, **kwargs):
	classifier_name = reduce(lambda x,y: x+y, re.findall('[A-Z][^a-z]*', str(classifier).strip("'>")))
	combs = value_combinations(dic_param_vals)
	metrics = pd.Series(["accuracy","precision","recall","f1","auc_roc","average_precision_score","train_time","test_time"])
	results = pd.DataFrame(columns=metrics)
	for comb in combs:
		...
		for train_index, test_index in splits:
			cv_train = df.loc[train_index]
			cv_test = df.loc[test_index]
			cv_train, cv_test = preprocessing(cv_train, cv_test)
			x_cols = cv_train.columns[3:]
			y_pred, y_pred_prob, y_test, time_train, time_test = run_cv(cv_train, cv_test, x_cols, y_col, classifier, **comb)
			for threshold in list_threshold:
...
if __name__ == '__main__':
	...
	"EnglishMatch", "SpanishMatch", "MotherNurseAgeDiff", "whiteMatch", "hispanicMatch", "blackMatch"]
	...
	train_df = run_missing_indicator(train_df,missing_cols)

	# generate time-dependent boostrapping indexes
	bootstrap_percent = 0.8
	num_splits = 3
	num_folds = 2
	splits = []
	splits += bootstrap_split(train_df, 2007, 2008, bootstrap_percent, num_splits)
	splits += bootstrap_split(train_df, 2008, 2009, bootstrap_percent, num_splits)
	size_list = []
	...
	for classifier in classifiers:
		results = select_parameter_bstp(train_df, splits, classifier, y_col, dic_param_vals[classifier], list_threshold, "f1")
